...
message_txt = "Mit Tastendruck geht es los."
pre_dur = 3
exp_dur = 300
...
class DummyExp:
    def __init__(self):
        ...
        self.message = visual.TextStim(self.win, text=message_txt)
    ...
    def finish(self):
        logging.info('finish up')
        if self.state == 'exp':
            self.end_cue()
            core.wait(2*sound_cfg['duration'])
        #del self.message
        self.win.close()
        core.quit()

...
class UrgeIndicatorTest(unittest.TestCase):
    ...
    def test_validateConfiguration_withInvalidBGCol_raisesError(self):
        self.config['bg_col'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidBGFrameCol_raisesError(self):
        self.config['bg_frame_col'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidBGFrameWidth_raisesError(self):
        self.config['bg_frame_width'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)
        self.config['bg_frame_width'] = -0.5
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGHeight_raisesError(self):
        self.config['fg_height'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGWidth_raisesError(self):
        self.config['fg_width'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGCol_raisesError(self):
        self.config['fg_col'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGFrameCol_raisesError(self):
        self.config['fg_frame_col'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGFrameWidth_raisesError(self):
        self.config['fg_frame_width'] = 'invalid'
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)
        self.config['fg_frame_width'] = -0.5
        with self.assertRaises(AssertionError): UrgeIndicator.UrgeIndicator(self.window, self.config)

    def test_validateConfiguration_withInvalidFGOpacity_raisesError(self):
